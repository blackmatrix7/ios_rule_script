...
magicJS.unifiedPushUrl = magicJS.read('dingdongmaicai_unified_push_url') || magicJS.read('magicjs_unified_push_url');
  ...
  if (magicJS.isRequest && getCookieRegex.test(magicJS.request.url)){
    ...
    if (!!!cookie){
      ...
      if (checkInErr){
        ...
        subTitle = `🎉签到成功，连续签到${signSeries}天`;
        content = `获取积分${point}`;
        if (ticketMoney > 0) content += `，优惠券${ticketMoney}元`;
      }
      // 通知
